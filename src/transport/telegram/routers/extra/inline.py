...
from src.transport.telegram.filters.inline import InlineGradesFilter
...
@inject
@router.inline_query(InlineGradesFilter())
...
@inject
@router.inline_query(F.query.startswith(InlineQueryText.TEACHERS))
...
@inject
@router.inline_query(F.query.startswith(InlineQueryText.GRADES))
async def inline_grades(
    query: InlineQuery,
    schedule_service: FromDishka[ScheduleService],
):
    grade = query.query.removeprefix(InlineQueryText.GRADES).strip()
...
@inject
@router.inline_query(F.query.startswith(InlineQueryText.ROOMS))
async def inline_rooms(
    query: InlineQuery,
    schedule_service: FromDishka[ScheduleService],
):
    room = query.query.removeprefix(InlineQueryText.ROOMS).strip()
...
async def inline_day(
    ...
    if user.grade == 0 or not schedule:
        buttons = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    message_text=i18n.get("ntf-error.unknown")
                ),
            )
        ]
        return await query.answer(
            ...
            switch_pm_text=i18n.get("ntf-registration"),
        )
    bot_link = await bot_service.get_referral_url("inline-ref-btn")
    kbd = url_kb(**{bot_link: i18n.get("inline-ref-btn")})
    return await query.answer(
