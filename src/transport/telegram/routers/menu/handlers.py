...
from src.transport.telegram.filters.user import NewUserFilter
from src.transport.telegram.keyboards import (
    get_days_keyboard,
    get_register_grade_keyboard,
    get_time_keyboard,
)
...
async def on_start_dialog(
    ...
    if not schedule:
        ...
        await notification_service.notify_user(
            user,
            MessagePayloadDto(
                i18n_key="start-emoji",
                reply_markup=get_days_keyboard(schedule.list_days_names()),
                delete_after=None,
            ),
        )
    await dialog_manager.start(
...
async def on_start_command(
    ...
    await on_start_dialog(user, dialog_manager, notification_service, user_service)


@router.message(NewUserFilter())
async def on_new_user(message: Message, user: UserDto, dialog_manager: DialogManager) -> None:
    await dialog_manager.start(
        state=Register.GRADE,
        mode=StartMode.RESET_STACK,
        show_mode=ShowMode.DELETE_AND_SEND,
...
async def on_get_tomorrow(
    ...
    if weekday >= len(user_schedule.days):
        await callback.answer(i18n.get("ntf-schedule.weekend", today=False))
        return
...
async def on_get_week(
    ...
    await notification_service.notify_user(
        ...
        MessagePayloadDto(
            ...
            i18n_kwargs={"text": user_schedule.text(i18n)},
            reply_markup=get_time_keyboard(display=True, week=True),
            delete_after=None,
        ),
    )
    return
...
async def on_day_request(
    ...
    user: UserDto = dialog_manager.middleware_data[USER_KEY]
    user_schedule = await user_service.get_schedule(user)
    await dialog_manager.switch_to(MainMenu.MAIN, show_mode=ShowMode.EDIT)
    ...
    list_days = user_schedule.list_days(i18n)
    if message.text not in list_days:
        await notification_service.notify_user(
            user,
            MessagePayloadDto(
    ...
    day_index = list_days.index(message.text)
...
async def on_room_request(
    ...
    if not message.text or not message.text.startswith(InlineQueryText.ROOMS):
        ...
        return
    room_schedule_id = int(message.text.removeprefix(InlineQueryText.ROOMS + "_").strip())
    ...
    await notification_service.notify_user(
        ...
        MessagePayloadDto(
            i18n_key="ntf-schedule.day",
            i18n_kwargs={"day": message.text, "text": room_schedule.text(i18n)},
            reply_markup=get_time_keyboard(),
            delete_after=None,
        ),
    )
...
async def on_free_rooms_day_selected(
    ...
    await notification_service.notify_user(
        ...
        MessagePayloadDto(
            ...
            i18n_kwargs={"day": WeekDay(day_number + 1).get_text(i18n), "text": free_rooms},
            delete_after=None,
        ),
    )
...
async def on_set_student(
    ...
    user: UserDto = dialog_manager.middleware_data[USER_KEY]
    user.default_child = student_id
    await user_service.update(user)

    await callback.answer(text=i18n.get("ntf-success.setting"))
    await dialog_manager.switch_to(MainMenu.MAIN, show_mode=ShowMode.EDIT)
