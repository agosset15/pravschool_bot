...
class BotService:
    ...
    async def get_mini_app_url(self, start_data: Optional[str] = None) -> str:
        ...
        if start_data:
            url += f"?startapp={quote(start_data)}"
        return url
    ...
    def validate_webapp_data(self, init_data: str) -> WebAppInitData:
        return safe_parse_webapp_init_data(token=self.bot.token, init_data=init_data)
