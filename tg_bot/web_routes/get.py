...
async def getdb_rasp_today(request: Request):
    ...
    if datetime.datetime.now() >= datetime.datetime(datetime.date.today().year, datetime.date.today().month, datetime.date.today().day) + datetime.timedelta(hours=14):
        ...
        tomorrow = True
    if date > 4:
        return json_response({"ok": "True", "rasp": "Выходной!", "tomorrow": f"{tomorrow}"}, dumps=json.JSONEncoder(ensure_ascii=False), text=str({'ok': 'True', 'rasp': 'Выходной!', 'tomorrow': tomorrow}), body=str({'ok': 'True', 'rasp': 'Выходной!', 'tomorrow': tomorrow}).encode())
    if usr.isTeacher is True:
